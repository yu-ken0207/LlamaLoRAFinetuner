...
class AlfredTWEnv(object):
    ...
    def collect_game_files(self, verbose=False):
        ...
        import ast  # 或使用 eval，這裡 ast 更安全

        # 假設 config 是你的配置字典
        task_types_str = self.config['env']['task_types']

        # 檢查是否是字符串並且是列表的形式
        if isinstance(task_types_str, str) and task_types_str.startswith('[') and task_types_str.endswith(']'):
            self.config['env']['task_types'] = ast.literal_eval(task_types_str)  # 將字符串轉換為列表
